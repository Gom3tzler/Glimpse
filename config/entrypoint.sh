...
# Create the cron job
echo "$CRON_SCHEDULE root cd /app && python /app/scripts/plex_data_fetcher.py --url \"$PLEX_URL\" --token \"$PLEX_TOKEN\" --output /app/data >> /var/log/cron.log 2>&1" > /etc/cron.d/plex-cron
...
python /app/scripts/plex_data_fetcher.py --url "$PLEX_URL" --token "$PLEX_TOKEN" --output /app/data
